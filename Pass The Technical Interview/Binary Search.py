def recursive_binary_search(sorted_list, left_pointer, right_pointer, target):
    ...
    if mid_val > target:
        ...
        return recursive_binary_search(sorted_list, left_pointer, mid_idx, target)
    if mid_val < target:
        # we reduce the sub-list by passing in a new left_pointer
        return recursive_binary_search(sorted_list, mid_idx + 1, right_pointer, target)


values = [77, 80, 102, 123, 288, 300, 540]
start_of_values = 0
end_of_values = len(values)
result = recursive_binary_search(values, start_of_values, end_of_values, 288)
